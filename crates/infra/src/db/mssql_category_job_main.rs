...
impl CategoryJobMainRepository for MssqlCategoryJobMainRepository {
    ...
    async fn create(
        ...
        let rows = exec_sp(
            &self.pool,
            "EXEC dbo.SP_CATEGORY_JOB_MAIN_CREATE \
                @p_category_job_main_name = @P1, \
                @p_category_job_main_icon_style = @P2, \
                @p_category_job_main_icon_line = @P3, \
                @p_category_job_main_img_path = @P4, \
                @p_category_job_main_priority = @P5, \
                @p_create_by = @P6",
            params,
        )
        .await?;
        ...
        let row = rows.first().ok_or_else(|| {
            RepoError::Backend(
                "SP_CATEGORY_JOB_MAIN_CREATE returned no row (driver/protocol mismatch)".into(),
            )
        })?;
